                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {(zonePhotos[zone.id] || []).map((photo) => (
                    <a key={photo.id} href={photo.photo_url} target="_blank" rel="noreferrer" style={{ position: 'relative', display: 'block' }}>
                      ...
                      />
                      <span style={{ position: 'absolute', left: '5px', bottom: '5px', fontSize: '10px', background: 'rgba(255,255,255,0.72)', color: '#0f172a', padding: '2px 5px', borderRadius: '4px' }}>
                        {new Date(photo.created_at || Date.now()).toLocaleString()} • Logo
                      </span>
                    </a>
                  ))}
                </div>
