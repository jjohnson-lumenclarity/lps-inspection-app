...
<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-16">
  {projects.map((project: any) => (
    <div 
      key={project.id}
      className="group relative bg-white/80 backdrop-blur-xl overflow-hidden rounded-3xl shadow-2xl hover:shadow-3xl border border-white/50 hover:border-blue-200/70 hover:scale-[1.02] hover:-translate-y-2 transition-all duration-500 cursor-pointer max-w-sm mx-auto"
      onClick={() => {
    ...
    >
      {/* Image */}
      <div 
        className="h-48 w-full bg-gradient-to-br from-blue-500 to-indigo-600 relative overflow-hidden group-hover:brightness-110 transition-all"
        style={{
          backgroundImage: `url(${project.photo_url || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800&auto=format&fit=crop&q=80'})`,
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
          <div className="text-white drop-shadow-2xl">
            <div className="text-2xl font-black mb-1">{project.project_areas?.length || 0} zones</div>
            <div className="text-sm font-medium">Click to pin</div>
          </div>
        </div>
      </div>
      ...
      {/* Content */}
      <div className="p-6">
        <h2 className="text-xl font-black mb-3 text-gray-900 leading-tight line-clamp-2">{project.title}</h2>
        <p className="text-lg text-gray-600 mb-4 leading-relaxed line-clamp-2">{project.address}</p>
        <div className="flex items-center justify-between">
          <span className="px-4 py-2 bg-gradient-to-r from-emerald-400 to-teal-500 text-white text-sm font-bold rounded-xl shadow-lg">
            {project.status?.toUpperCase() || 'ACTIVE'}
          </span>
          <div className="text-xl font-black text-blue-600">{project.project_areas?.length || 0}</div>
        </div>
      </div>
    </div>
  ))}
