...
export default function InspectionsPage() {
  ...
  }, [fetchZonePhotos, loadZoneDetails, loadZoneNotes]);
                      ...
                      {pin.id && (
                        <>
                          ...
                          <div className="mt-3" style={{ marginTop: '12px' }}>
                            ...
                            {pin.id && (
                              <div className="mt-2 grid grid-cols-1 gap-2" style={{ marginTop: '10px' }}>
                                <p className="text-xs font-semibold text-slate-700">Zone detail panel</p>
                                <input
                                  placeholder="Zone type"
                                  value={zoneDetailsById[pin.id]?.zoneType || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'zoneType', event.target.value)}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <div className="grid grid-cols-2 gap-2">
                                  <input
                                    placeholder="Count / quantity"
                                    value={zoneDetailsById[pin.id]?.quantity || ''}
                                    onChange={(event) => updateZoneDetailField(pin.id!, 'quantity', event.target.value)}
                                    style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                  />
                                  <input
                                    placeholder="Access difficulty"
                                    value={zoneDetailsById[pin.id]?.accessDifficulty || ''}
                                    onChange={(event) => updateZoneDetailField(pin.id!, 'accessDifficulty', event.target.value)}
                                    style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                  />
                                </div>
                                <input
                                  placeholder="Height / lift needed"
                                  value={zoneDetailsById[pin.id]?.heightLiftNeeded || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'heightLiftNeeded', event.target.value)}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <textarea
                                  placeholder="Existing condition"
                                  value={zoneDetailsById[pin.id]?.existingCondition || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'existingCondition', event.target.value)}
                                  rows={2}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <textarea
                                  placeholder="Recommended work"
                                  value={zoneDetailsById[pin.id]?.recommendedWork || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'recommendedWork', event.target.value)}
                                  rows={2}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <textarea
                                  placeholder="Material notes"
                                  value={zoneDetailsById[pin.id]?.materialNotes || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'materialNotes', event.target.value)}
                                  rows={2}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <input
                                  placeholder="Urgency / safety risk"
                                  value={zoneDetailsById[pin.id]?.urgencyRisk || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'urgencyRisk', event.target.value)}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                <textarea
                                  placeholder="Typed note or voice summary"
                                  value={zoneDetailsById[pin.id]?.typedOrVoiceNotes || ''}
                                  onChange={(event) => updateZoneDetailField(pin.id!, 'typedOrVoiceNotes', event.target.value)}
                                  rows={2}
                                  style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '7px 8px', fontSize: '12px' }}
                                />
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {recordingZoneId === pin.id ? (
                                    <button
                                      ...
                                      style={{ backgroundColor: '#dc2626', color: '#fff', borderRadius: '6px', padding: '6px 10px' }}
                                    >
                                      Stop voice note
                                    </button>
                                  ) : (
                                    <button
                                      type="button"
                                      onClick={() => void startVoiceRecording(pin.id!)}
                                      className="rounded bg-teal-700 px-3 py-1 text-xs font-semibold text-white"
                                      style={{ backgroundColor: '#0f766e', color: '#fff', borderRadius: '6px', padding: '6px 10px' }}
                                    >
                                      Record voice note
                                    </button>
                                  )}
                                  <button
                                    type="button"
                                    disabled={savingZoneDetailId === pin.id || !selectedProject}
                                    onClick={() => selectedProject && void saveZoneDetails(selectedProject.id, pin.id!)}
                                    className="rounded bg-cyan-700 px-3 py-1 text-xs font-semibold text-white disabled:bg-slate-300"
                                    style={{ backgroundColor: '#0e7490', color: '#fff', borderRadius: '6px', padding: '6px 10px' }}
                                  >
                                    {savingZoneDetailId === pin.id ? 'Saving details…' : 'Save zone details'}
                                  </button>
                                </div>
                                {zoneDetailsById[pin.id]?.voiceNoteUrl && (
                                  <audio controls style={{ width: '100%', marginTop: '4px' }} src={zoneDetailsById[pin.id]?.voiceNoteUrl} />
                                )}
                              </div>
                            )}
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.72)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
            <div style={{ width: 'min(960px, 100%)', maxHeight: '90vh', overflow: 'auto', background: '#fff', borderRadius: '12px', padding: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <h3 style={{ margin: 0, color: '#0f172a' }}>Zone photo markup (local)</h3>
