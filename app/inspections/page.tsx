  ...
  return (
    <div style={{ padding: 24, background: '#f8fafc', minHeight: '100vh' }}>
      <header style={{ border: '1px solid #e2e8f0', borderRadius: 16, background: '#fff', padding: 24, boxShadow: '0 1px 2px rgba(15,23,42,.06)' }}>
        ...
        <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr', gap: 14 }}>
          ...
          <button onClick={() => setDrawingEnabled((v) => !v)} style={{ borderRadius: 10, border: '1px solid #cbd5e1', background: drawingEnabled ? '#dcfce7' : '#fff', fontWeight: 700 }}> {drawingEnabled ? 'Drawing: ON' : 'Enable Drawing'} </button>
        </div>

        <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1.5fr .6fr auto', gap: 10, alignItems: 'center' }}>
          ...
          <input type="number" min={16} max={22} value={mapZoom} onChange={(e) => setMapZoom(Number(e.target.value) || 19)} style={inputStyle} />
          <button onClick={saveAerialFrameToProject} style={{ borderRadius: 10, border: 'none', background: '#2563eb', color: '#fff', padding: '12px 16px', fontWeight: 800 }}>Save frame to project</button>
        </div>

        {mapPreview && (
          <div style={{ marginTop: 10, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <img src={mapPreview} alt="Map preview" style={{ width: 230, height: 130, objectFit: 'cover', borderRadius: 8, border: '1px solid #cbd5e1' }} />
            {(projectMeta[selectedProjectId]?.aerialImages || []).slice(0, 5).map((img) => (
