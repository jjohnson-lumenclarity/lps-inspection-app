...
const getStaticPreviewUrl = (address: string, zoom: number, size: '1200x800' | '1000x1400') => {
...
export default function Dashboard() {
  ...
  const [previewSize, setPreviewSize] = useState<'1200x800' | '1000x1400'>('1200x800');
  const formRef = useRef<HTMLDivElement | null>(null);

  const canSubmitProject = newProject.title.trim().length > 0 && newProject.address.trim().length > 0;
  const previewUrl = newProject.address.trim() ? getStaticPreviewUrl(newProject.address, previewZoom, previewSize) : null;
  ...
  const addProject = async () => {
    ...
    setNewProject({
      title: '', description: '', address: '', status: 'In Progress', contactName: '', phone: '', email: '',
      inspectionDate: new Date().toISOString().slice(0, 10), dueDate: '', weather: 'Clear', temperature: '',
    });
  };
  ...
  const downloadSnapshot = async () => {
    ...
    const url = getStaticPreviewUrl(newProject.address, previewZoom, previewSize);
    const filename = `${newProject.title.trim() || 'project'}-overhead-map-z${previewZoom}-${previewSize}.png`.replace(/\s+/g, '-').toLowerCase();
    ...
    <main style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px' }}>
          <input placeholder="Company Name (required)" value={newProject.title} onChange={(e) => setNewProject({ ...newProject, title: e.target.value })} style={{ padding: '12px 14px', border: '1px solid #D1D5DB', borderRadius: '8px', fontSize: '15px' }} />
          ...
          <input placeholder="Contact Name" value={newProject.contactName} onChange={(e) => setNewProject({ ...newProject, contactName: e.target.value })} style={{ padding: '12px 14px', border: '1px solid #D1D5DB', borderRadius: '8px', fontSize: '15px' }} />
      ...
      {previewUrl && (
        <div style={{ marginBottom: '30px', background: '#fff', borderRadius: '12px', padding: '16px', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <label style={{ color: '#475569', fontSize: '13px' }}>
                Orientation{' '}
                <select value={previewSize} onChange={(e) => setPreviewSize(e.target.value as '1200x800' | '1000x1400')} style={{ padding: '6px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
                  <option value="1200x800">Landscape</option>
          ...
          <img src={previewUrl} alt="Satellite preview" style={{ width: '100%', maxHeight: '760px', objectFit: 'contain', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', background: '#e2e8f0' }} />
          <p style={{ marginTop: '8px', color: '#64748b', fontSize: '13px' }}>Download uses exactly this preview image (same zoom and orientation).</p>
        </div>
      )}
            ...
            return (
              <div key={project.id} style={{ padding: '25px', border: '1px solid #E5E7EB', borderRadius: '12px', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', marginBottom: '16px' }}>
                  <span style={{ padding: '4px 12px', background: project.status === 'In Progress' ? '#f59e0b' : project.status === 'Submitted' ? '#2563eb' : '#64748b', color: 'white', borderRadius: '20px', fontSize: '14px', fontWeight: 500 }}>{project.status}</span>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <Link href="/inspections" style={{ padding: '6px 10px', borderRadius: '6px', border: '1px solid #2563eb', color: '#2563eb', textDecoration: 'none', fontSize: '12px', fontWeight: 600 }}>Open Inspection</Link>
                ...
                {project.photo_url && <div style={{ marginBottom: '12px' }}><img src={project.photo_url} alt="Project" style={{ width: '100%', maxHeight: '200px', objectFit: 'cover', borderRadius: '8px' }} /></div>}
