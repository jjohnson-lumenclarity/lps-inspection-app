...
const getStaticPreviewUrl = (address: string, zoom: number) => {
  ...
  return `https://maps.googleapis.com/maps/api/staticmap?size=1600x1200&maptype=satellite&zoom=${zoom}&markers=color:0x374151|${query}&key=${googleMapsApiKey}`;
};
    ...
    <main style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>
      ...
      {previewUrl && (
        <div style={{ marginBottom: '30px', background: '#fff', borderRadius: '12px', padding: '16px', border: '1px solid #e2e8f0' }}>
          ...
          <p style={{ marginTop: '8px', color: '#64748b', fontSize: '13px' }}>Use the map above to pan/zoom visually. Download uses the same address + zoom level for a clean overhead snapshot.</p>
        </div>
      )}
            ...
            return (
              <div key={project.id} style={{ padding: '25px', border: '1px solid #E5E7EB', borderRadius: '12px', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ marginBottom: '10px' }}>
                  <span style={{ padding: '4px 12px', background: project.status === 'In Progress' ? '#f59e0b' : project.status === 'Submitted' ? '#2563eb' : '#64748b', color: 'white', borderRadius: '20px', fontSize: '14px', fontWeight: 500, display: 'inline-flex' }}>{project.status}</span>
                </div>
                <div style={{ display: 'flex', gap: '6px', marginBottom: '14px', flexWrap: 'wrap' }}>
                  <Link href={`/dashboard?edit=${project.id}`} style={{ padding: '6px 10px', borderRadius: '6px', border: '1px solid #2563eb', color: '#2563eb', textDecoration: 'none', fontSize: '12px', fontWeight: 600 }}>Edit Project Info</Link>
