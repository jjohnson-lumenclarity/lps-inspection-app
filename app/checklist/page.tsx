...
export default function ChecklistPage() {
  ...
  const renderQuestionCard = (item: ChecklistItem) => {
    ...
    return (
      ...
      >
        <p style={{ margin: 0, fontSize: '14px', fontWeight: 600, color: '#0f172a', lineHeight: 1.45 }}>{item.question}</p>
        ...
        {(expanded || row.response === 'na' || isActive) && (
          <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '8px' }}>
            ...
            {!compact && (
              <textarea value={row.notes} onChange={(e) => updateItem(item.id, { notes: e.target.value })} placeholder="Notes" style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '8px 10px', fontSize: '13px', minHeight: '72px', gridColumn: '1 / -1' }} />
            )}
          </div>
        )}
      </article>
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif', padding: '92px 16px 20px calc(var(--sidebar-width, 88px) + 16px)' }}>
      <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
        <div style={{ border: '1px solid #e2e8f0', borderRadius: '16px', background: '#fff', padding: '16px', boxShadow: '0 8px 24px rgba(15,23,42,0.06)' }}>
          ...
          <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr auto', gap: '10px' }}>
              ...
              {categories.map((category) => {
                ...
                return (
                  <button key={category.id} type="button" onClick={() => { setActiveCategoryId(category.id); setActiveQuestionId(category.items[0].id); }} style={{ width: '100%', textAlign: 'left', border: active ? '1px solid #60a5fa' : '1px solid #e2e8f0', borderRadius: '10px', background: active ? '#eff6ff' : '#fff', padding: '10px', marginBottom: '8px' }}>
                    ...
                    <p style={{ margin: '2px 0 0 0', fontSize: '11px', color: '#b45309' }}>Issues: {stats.issues} • Photos: {stats.linkedPhotos}</p>
                  </button>
                );
              })}
            </aside>
