...
export default function ChecklistPage() {
  ...
  const [finalReview, setFinalReview] = useState<Record<string, string>>({
    major_areas_covered: '',
    enough_photos: '',
    blocked_access: '',
    missing_dimensions: '',
  ...
  const loadChecklist = useCallback(
    async (projectId: string) => {
      ...
      if (data) {
        ...
        setFinalReview({
          major_areas_covered: record.checklist_data?.major_areas_covered || '',
          enough_photos: record.checklist_data?.enough_photos || '',
          blocked_access: record.checklist_data?.blocked_access || '',
          missing_dimensions: record.checklist_data?.missing_dimensions || '',
          special_equipment_needed: record.checklist_data?.special_equipment_needed || '',
          safety_issues: record.checklist_data?.safety_issues || '',
        });
      } else {
        setDefaultAnswers();
        setOverallNotes('');
  ...
  const handleSave = async () => {
    ...
    const allReviewAnswered = Object.values(finalReview).every((value) => value === 'yes' || value === 'no');
    if (!allReviewAnswered) {
      setStatusMessage('Complete the pre-submit review checklist (all Yes/No questions) before saving.');
      return;
    }
    ...
    const payload = {
      ...
      checklist_data: { ...answers, ...finalReview },
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-8" style={{ minHeight: '100vh', backgroundColor: '#f8fafc', paddingTop: '96px', paddingLeft: '20px', fontFamily: 'Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif' }}>
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" style={{ border: '1px solid #dbe3ea', borderRadius: '14px', backgroundColor: '#fff', padding: '20px', boxShadow: '0 8px 24px rgba(15,23,42,0.06)' }}>
          <h2 className="text-lg font-bold text-slate-900">Complete review checklist (required before submit)</h2>
          <div className="mt-4 grid gap-3">
            {[
              { key: 'major_areas_covered', label: 'All major areas covered?' },
              { key: 'enough_photos', label: 'Enough photos captured?' },
              { key: 'blocked_access', label: 'Any blocked access?' },
              { key: 'missing_dimensions', label: 'Any missing dimensions?' },
              { key: 'special_equipment_needed', label: 'Any special equipment needed?' },
              { key: 'safety_issues', label: 'Any safety issues?' },
            ].map((item) => (
              <div key={item.key} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px' }}>
                <p className="text-sm font-semibold text-slate-800">{item.label}</p>
                <div className="mt-2 flex gap-2">
                  {['yes', 'no'].map((value) => {
                    const selected = finalReview[item.key] === value;
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => setFinalReview((prev) => ({ ...prev, [item.key]: value }))}
                        style={{
                          backgroundColor: selected ? (value === 'yes' ? '#059669' : '#dc2626') : '#fff',
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" style={{ border: '1px solid #dbe3ea', borderRadius: '14px', backgroundColor: '#fff', padding: '20px', boxShadow: '0 8px 24px rgba(15,23,42,0.06)' }}>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            {selectedProjectId && (
              <Link href={`/quote-summary?projectId=${selectedProjectId}`} className="rounded bg-slate-800 px-4 py-2 text-sm font-semibold text-white" style={{ backgroundColor: '#1e293b', color: '#fff', borderRadius: '8px', padding: '10px 14px', textDecoration: 'none' }}>
                Open Quote Summary
              </Link>
            )}
          </div>
