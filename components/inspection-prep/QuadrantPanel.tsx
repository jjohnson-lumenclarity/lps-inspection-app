...
export function QuadrantPanel({ quadrants, selected, zoneState, photos, onSelect, onPrev, onNext, onNotesChange, onUpload, uploading, projectId, onStartRecording, onStopRecording, recording, drawingEnabled, onToggleDrawing }: {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', minWidth: 0, border: '1px solid #cbd5e1', borderRadius: 8, padding: 10, fontSize: 14, background: '#fff' };
  return (
      ...
      {selected ? (
        <div style={{ marginTop: 12, display: 'grid', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={onToggleDrawing} style={{ border: '1px solid #cbd5e1', borderRadius: 8, padding: '6px 10px', background: drawingEnabled ? '#dcfce7' : '#fff' }}>{drawingEnabled ? 'Drawing ON' : 'Draw'}</button>
          </div>
