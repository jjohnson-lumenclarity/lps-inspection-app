  ...
  return (
    <div style={{ border: '1px solid #e2e8f0', borderRadius: 16, background: '#fff', padding: 16, boxShadow: '0 1px 2px rgba(15,23,42,.06)' }}>
      ...
      <div
        ...
        style={{
          position: 'relative', minHeight: 640, cursor: drawingEnabled ? 'crosshair' : 'pointer', overflow: 'hidden', borderRadius: 12, border: '1px solid #e2e8f0', backgroundColor: '#f1f5f9',
          backgroundImage: imageUrl ? `url(${imageUrl})` : undefined, backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat',
        }}
      >
        ...
        >
          {lines.map((line, i) => line.length > 1 ? <polyline key={i} points={line.map((p) => `${p.x},${p.y}`).join(' ')} fill="none" stroke="#22c55e" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /> : null)}
        </svg>
      ...
      <p style={{ marginTop: 8, marginBottom: 0, fontSize: 12, color: '#64748b' }}>Click image to create quadrants. Enable drawing to annotate roof plan.</p>
    </div>
  );
}
