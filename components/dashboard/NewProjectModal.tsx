...
export function NewProjectModal({ open, draft, onClose, setDraft, onSave, editing }: {
  open: boolean;
  draft: DraftProject;
  onClose: () => void;
  ...
  const inputStyle: React.CSSProperties = { border: '1px solid #cbd5e1', borderRadius: 10, padding: '10px 12px', fontSize: 16, width: '100%', minWidth: 0 };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(2,6,23,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 1220, maxHeight: 'calc(100vh - 32px)', overflowY: 'auto', borderRadius: 16, background: '#fff', padding: 20, boxShadow: '0 16px 60px rgba(0,0,0,.25)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ margin: 0, fontSize: 34, color: '#0f172a' }}>{editing ? 'Edit Project' : 'Add New Project'}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginTop: 14 }}>
          <button onClick={onSave} style={{ border: 'none', borderRadius: 10, padding: '10px 16px', background: '#2563eb', color: '#fff', fontWeight: 800, fontSize: 18 }}>{editing ? 'Save Changes' : 'Add Project'}</button>
          <button onClick={onClose} style={{ border: 'none', borderRadius: 8, padding: '10px 14px', background: '#dc2626', color: '#fff', fontWeight: 700 }}>Cancel</button>
        </div>
      </div>
    </div>
  );
