        ...
        <div key={index} style={{ border: '1px solid #e2e8f0', borderRadius: 14, background: '#fff', padding: 12 }}>
          <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))' }}>
            <label style={{ display: 'grid', gap: 6 }}>
              ...
              <input value={row.worker_name} onChange={(e) => onChange(index, { worker_name: e.target.value })} style={fieldStyle} />
            </label>
            <label style={{ display: 'grid', gap: 6 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>Role / Class</span>
              <input value={row.role_class} onChange={(e) => onChange(index, { role_class: e.target.value })} style={fieldStyle} />
            </label>
            <label style={{ display: 'grid', gap: 6 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>ST</span>
              <input type="number" min={0} step="1" value={row.st_hours} onChange={(e) => onChange(index, { st_hours: Number.parseInt(e.target.value, 10) || 0 })} style={fieldStyle} />
            </label>
            <label style={{ display: 'grid', gap: 6 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>OT</span>
              <input type="number" min={0} step="1" value={row.ot_hours} onChange={(e) => onChange(index, { ot_hours: Number.parseInt(e.target.value, 10) || 0 })} style={fieldStyle} />
            </label>
