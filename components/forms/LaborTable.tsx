        ...
        <div key={index} style={{ border: '1px solid #e2e8f0', borderRadius: 14, background: '#fff', padding: 12 }}>
          <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))' }}>
            ...
            <label style={{ display: 'grid', gap: 6 }}>
              ...
              <input type="number" min={0} step="0.25" value={row.st_hours} onChange={(e) => onChange(index, { st_hours: Number(e.target.value) || 0 })} style={fieldStyle} />
            </label>
            <label style={{ display: 'grid', gap: 6 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>OT</span>
              <input type="number" min={0} step="0.25" value={row.ot_hours} onChange={(e) => onChange(index, { ot_hours: Number(e.target.value) || 0 })} style={fieldStyle} />
            </label>
