...
export function DailyFieldLaborReportEditor({ reportId }: { reportId?: string }) {
  ...
  const completion = useMemo(() => ({
    ...
    photos: true,
    signatures: Boolean(form.guardian_signature_data && (form.client_not_available || form.customer_signature_data)),
    review: false,
  ...
  const validationErrors = useMemo(() => ({
    ...
    guardian_signature_data: !form.guardian_signature_data ? 'Guardian signature is required before submit.' : '',
    customer_signature_data: !form.client_not_available && !form.customer_signature_data ? 'Customer / GC signature is required unless marked unavailable.' : '',
  }), [form]);
  ...
  const save = useCallback(async (nextStatus: ReportStatus, silent = false) => {
    ...
    const payload = {
      ...
      guardian_signed_at: form.guardian_signature_data ? form.guardian_signed_at || new Date().toISOString() : null,
      customer_signed_at: form.customer_signature_data ? form.customer_signed_at || new Date().toISOString() : null,
    };
  ...
  return (
    <div style={{ padding: 20, background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <section style={{ ...cardStyle, marginBottom: 12 }}>
        <div style={{ borderBottom: '1px solid #e2e8f0', padding: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {SECTION_ORDER.map((section) => (
            <button
              ...
              style={{
                ...
                padding: '8px 12px',
                fontWeight: 600,
                background: activeSection === section.key ? '#0f172a' : '#fff',
                color: activeSection === section.key ? '#fff' : '#0f172a',
              }}
            >
              {section.title.replace('Work Performed / ', '')}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: 14 }}>
          ...
          {activeSection === 'signatures' && (
            <div style={{ display: 'grid', gap: 12 }}>
              <SignaturePadField
                ...
                error={hasAttemptedSubmit ? validationErrors.guardian_signature_data : ''}
                onChange={({ value, signedAt }) => setForm((prev) => ({ ...prev, guardian_signature_data: value, guardian_signed_at: signedAt }))}
              />

              <div style={{ border: '1px solid #e2e8f0', borderRadius: 14, padding: 14, background: '#fff' }}>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 600 }}>
                  <input type="checkbox" checked={form.client_not_available} onChange={(e) => setForm((prev) => ({ ...prev, client_not_available: e.target.checked, customer_signature_data: e.target.checked ? '' : prev.customer_signature_data, customer_signed_at: e.target.checked ? '' : prev.customer_signed_at }))} />
              ...
              <SignaturePadField
                ...
                value={form.customer_signature_data}
                signedAt={form.customer_signed_at}
                error={hasAttemptedSubmit ? validationErrors.customer_signature_data : ''}
                onChange={({ value, signedAt }) => setForm((prev) => ({ ...prev, customer_signature_data: value, customer_signed_at: signedAt }))}
              />
            </div>
          )}
          ...
          {activeSection === 'review' && (
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              <ReviewLine label="Guardian signature" value={form.guardian_signature_data ? 'Captured' : 'Missing'} />
              <ReviewLine label="Client availability" value={form.client_not_available ? 'Client / GC not available on site' : 'Client available'} />
              <ReviewLine label="Customer signature" value={form.client_not_available ? 'Not required' : form.customer_signature_data ? 'Captured' : 'Missing'} />
          ...
          <SectionNavButtons activeSection={activeSection} onChange={setActiveSection} />
        </div>
      </section>
...
function SectionNavButtons({ activeSection, onChange }: { activeSection: SectionKey; onChange: (section: SectionKey) => void }) {
  const currentIndex = SECTION_ORDER.findIndex((section) => section.key === activeSection);
  const prev = currentIndex > 0 ? SECTION_ORDER[currentIndex - 1] : null;
  const next = currentIndex < SECTION_ORDER.length - 1 ? SECTION_ORDER[currentIndex + 1] : null;

  return (
    <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
      <button
        type="button"
        disabled={!prev}
        onClick={() => prev && onChange(prev.key)}
        style={{ borderRadius: 10, border: '1px solid #cbd5e1', background: '#fff', padding: '8px 12px', opacity: prev ? 1 : 0.5 }}
      >
        ← Back
      </button>
      <button
        ...
        style={{ borderRadius: 10, border: '1px solid #0f172a', background: '#0f172a', color: '#fff', padding: '8px 12px', opacity: next ? 1 : 0.5 }}
      >
        Next →
      </button>
    </div>
